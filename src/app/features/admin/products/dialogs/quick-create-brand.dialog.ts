...
import { Brand } from '../../../../core/models/product-catalog.model';

export interface QuickCreateBrandData { initialName: string }

@Component({
  selector: 'app-quick-create-brand-dialog',
  standalone: true,
  imports: [
    ReactiveFormsModule,
  ...
  template: `
    <div cdkDrag cdkDragRootElement=".cdk-overlay-pane">
      <h2 mat-dialog-title cdkDragHandle class="qcd-title">
        <mat-icon>add_circle</mat-icon> Crear marca
      </h2>
...
export class QuickCreateBrandDialogComponent {
  private readonly dialogRef = inject(MatDialogRef<QuickCreateBrandDialogComponent>);
  private readonly data      = inject<QuickCreateBrandData>(MAT_DIALOG_DATA);
  ...
  submit(): void {
    ...
    this.service.create(this.form.value.name!).subscribe({
      ...
      error: (err: any) => {
        this.saving.set(false);
        this.error.set(err?.error?.message ?? 'Error al crear la marca.');
      },
    });
  }
