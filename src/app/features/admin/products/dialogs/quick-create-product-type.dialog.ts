...
import { ProductType } from '../../../../core/models/product-catalog.model';

export interface QuickCreateProductTypeData { initialName: string }

@Component({
  selector: 'app-quick-create-product-type-dialog',
  standalone: true,
  imports: [
    ReactiveFormsModule,
  ...
  template: `
    <div cdkDrag cdkDragRootElement=".cdk-overlay-pane">
      <h2 mat-dialog-title cdkDragHandle class="qcd-title">
        <mat-icon>add_circle</mat-icon> Crear tipo de artículo
      </h2>
...
export class QuickCreateProductTypeDialogComponent {
  private readonly dialogRef = inject(MatDialogRef<QuickCreateProductTypeDialogComponent>);
  private readonly data      = inject<QuickCreateProductTypeData>(MAT_DIALOG_DATA);
  ...
  submit(): void {
    ...
    this.service.create(this.form.value.code!, this.form.value.name!).subscribe({
      ...
      error: (err: any) => {
        this.saving.set(false);
        this.error.set(err?.error?.message ?? 'Error al crear el tipo de artículo.');
      },
    });
  }
