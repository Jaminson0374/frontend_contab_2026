...
@Component({
  ...
  imports: [
    ...
    MatTabsModule, MatFormFieldModule, MatInputModule, MatSelectModule, MatAutocompleteModule,
    MatButtonModule, MatButtonToggleModule, MatCheckboxModule, MatDialogModule,
    MatDatepickerModule, MatNativeDateModule,
    MatIconModule, MatTooltipModule, MatProgressSpinnerModule, MatDividerModule,
  ],
...
export class ThirdPartyFormComponent implements OnInit {
  ...
  private readonly numIdentificationSig  = toSignal(this.form.get('numIdentification')!.valueChanges,
                                { initialValue: '' });
  ...
  readonly requiresDv = computed(() => {
    ...
    return this.catalogService.identificationTypes.value()?.find(t => t.id === id)?.requiresDv ?? false;
  });
  ...
  readonly pageTitle = computed(() => {
    switch (this.mode()) {
      case 'new':  return 'Nuevo tercero';
      case 'edit': return 'Editando tercero';
      default:     return 'Detalle de tercero';
    }
  });
  ...
  constructor() {
    ...
    effect(() => {
      ...
      this.form.get('dv')!.setValue(
        this.requiresDv() && numIdentification.length > 0 ? calculateNitDv(numIdentification) : '',
        { emitEvent: false }
      );
    });
    ...
    this.route.paramMap.pipe(takeUntilDestroyed(this.destroyRef)).subscribe(params => {
      ...
      if (!id) {
        ...
        this.service.getById(id).subscribe({
          next:  data => { this.loading.set(false); this.loadIntoForm(data); },
          error: ()   => { this.loading.set(false); this.error.set('Error al cargar el tercero.'); },
        });
      }
    });
  ...
  private resetForm(): void {
    this.form.reset({
      personType: 'JURIDICA', active: true,
      creditDays: 30, creditLimit: 0, taxRegime: 'ORDINARIO',
    });
    [this.catDisplay, this.idTypeDisplay, this.deptDisplay, this.cityDisplay]
  ...
  private enableForm(): void {
    ...
    [this.catDisplay, this.idTypeDisplay, this.deptDisplay, this.cityDisplay].forEach(c => c.enable());
  }

  private disableForm(): void {
    this.form.disable();
    [this.catDisplay, this.idTypeDisplay, this.deptDisplay, this.cityDisplay].forEach(c => c.disable());
  }
  ...
  openCreateCat(): void {
    this.dialog.open(QuickCreateCategoryDialogComponent, {
      disableClose: true,
      width: '400px',
  ...
  openCreateIdType(): void {
    this.dialog.open(QuickCreateIdTypeDialogComponent, {
      disableClose: true,
      width: '400px',
      data: { initialCode: this.idTypeFilter() },
    }).afterClosed().subscribe(idType => {
      if (!idType) return;
  ...
  save(): void {
    ...
    const request: ThirdPartyRequest = {
      ...
      employeeData: this.isEmployee() ? {
        ...
        birthDate:           v.empBirthDate
                               ? (v.empBirthDate as Date).toISOString().split('T')[0] : null,
